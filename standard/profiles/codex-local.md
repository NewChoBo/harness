...
Codex Local is an **on-demand execution runtime**, not a continuously-online Harness service. An operator may start or stop it independently of the durable work owner. A missing complete expectation operand is not evidence that no pulse was expected. `LOCAL_OFFLINE_OR_IDLE / NO_EXPECTED_PULSE` requires affirmative current evidence from the expectation authority/source that no current Codex execution expectation exists. If a possible or previously known expectation cannot be verified because its identity, currentness, expectation source, observer source, or bounded observation window is unavailable, classify it as `UNKNOWN / NEEDS_EVIDENCE`. An active claim/session alone does not establish `NO_SIGNAL` eligibility.
...
While such a verified expected Codex execution is active, material progress should preserve the current step, exact branch/SHA, actual validation result, blocker and next action. These execution-derived checkpoints are liveness evidence; do not create presence-only heartbeat spam.

If the checkpoint named by that expectation is absent after its bounded window, classify `NO_SIGNAL / RECONCILIATION_REQUIRED`, not `FAILED`. Use `LOCAL_OFFLINE_OR_IDLE / NO_EXPECTED_PULSE` only when an authoritative/current expectation source affirmatively establishes that no current expectation exists. If expectation identity/currentness, the expectation source, the observer source, or the bounded observation window is unavailable or unverifiable, classify `UNKNOWN / NEEDS_EVIDENCE`. Mere absence of a complete operand does not prove idle. Before retry or takeover, inspect current claim/queue state and exact Git/source side effects such as commit, push, branch/ref, PR, merge or external writes. Missing response is not proof that a write failed.
