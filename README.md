...
Repository correctness validation uses pnpm as the lockfile/package-manager source of truth:
...
The aggregate correctness gate checks root type/test/build, `@newchobo/harness-core`, every `packages/harness-workflow-*` preset, and structural public-boundary rules.

Formatting is tracked separately because the imported/packageized baseline currently contains pre-existing formatting debt:

```bash
pnpm format:check
# or combine formatting + correctness after the baseline is normalized
pnpm validate:strict
```

Do not hide semantic failures behind formatting churn, and do not claim formatting is clean unless `format:check` actually passes.
